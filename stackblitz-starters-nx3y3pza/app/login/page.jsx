...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: 'https://photostore-chi.vercel.app/dashboard' }
    })
  }
    ...
    try {
      if (isSignup) {
        const { error } = await supabase.auth.signUp({
          email,
          options: { data: { passkey: true } }
        })
        if (error) setError(error.message)
        else {
          const { error: pkError } = await supabase.auth.mfa.enroll({ factorType: 'webauthn' })
          if (pkError) setError(pkError.message)
          else setMessage('Passkey registered!')
        }
      } else {
        const { error } = await supabase.auth.signInWithPasskey({ email })
        if (error) setError(error.message)
        else router.push('/dashboard')
      }
    } catch (e) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: '380px' }}>
        <div style={{ marginBottom: '40px', textAlign: 'center' }}>
          <div style={{ fontSize: '28px', fontWeight: '600', letterSpacing: '-0.5px', marginBottom: '8px' }}>📷 PhotoStore</div>
          <p style={{ color: '#666', fontSize: '14px' }}>Your private media vault</p>
        </div>
...
/> {isSignup ? 'Register passkey' : 'Sign in with passkey'}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#444', fontSize: '12px' }}>or continue with email</span>
            <div style={{ flex: 1, height: '1px', background: '#222' }} />
          </div>
          ...
          {message && <p style={{ color: '#4ade80', fontSize: '13px', marginTop: '12px' }}>{message}</p>}

          <button onClick={handleSubmit} disabled={loading || !email || !password}
            style={{ width: '100%', marginTop: '16px', background: '#f0f0f0', color: '#0a0a0a', padding: '12px', fontWeight: '600', fontSize: '15px', borderRadius: '10px' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: '16px', fontSize: '13px', color: '#666' }}>
            ...
            <button onClick={() => { setIsSignup(!isSignup); setError(''); setMessage('') }}
              style={{ background: 'none', color: '#aaa', textDecoration: 'underline', padding: 0, fontSize: '13px' }}>
